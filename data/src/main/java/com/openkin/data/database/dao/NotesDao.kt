...
interface NotesDao {
    ...
    @Query("SELECT * FROM table_notes_database " +
            "WHERE (targetDate BETWEEN :startTime AND :endTime) AND archived = 0")
    ...
    @Query("SELECT COUNT(createDateMS) FROM table_notes_database " +
            "WHERE (targetDate BETWEEN :startTime AND :endTime) AND archived = 0")
    suspend fun getNotesCountByDateRange(startTime: Long, endTime: Long): Int

    @Query("SELECT COUNT(createDateMS) FROM table_notes_database " +
            "WHERE targetDate = :targetDate AND archived = 0")
